...
1. **Locate the .gsd/ directory** — check the current working directory (or `--path` argument):
   ```bash
   node "/Users/wesleysum/Desktop/EdgeOne/Little Projects/edgeone-qa-assistant/.claude/get-shit-done/bin/gsd-tools.cjs" from-gsd2 --dry-run
   ```
...
3. **Run the migration** after confirmation:
   ```bash
   node "/Users/wesleysum/Desktop/EdgeOne/Little Projects/edgeone-qa-assistant/.claude/get-shit-done/bin/gsd-tools.cjs" from-gsd2
   ```
